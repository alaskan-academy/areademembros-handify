...
export default async function AdminCoursesPage() {
  ...
  const [{ data: courses }, { data: categories }, { data: forums }, { data: showcaseRows }, { data: niches }] = await Promise.all([
    supabase
      .from("courses")
      .select("id, title, slug, description, price, checkout_codes, workload_hours, course_type, in_plan, has_certificate, published, category_id, forum_id, niche_id, thumbnail_url, checkout_url, position, category:categories(name), forum:forums(title, slug)")
      .order("position")
